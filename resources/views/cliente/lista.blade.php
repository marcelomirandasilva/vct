   ...
   <div class="x_panel modal-content ">
      ...
      <div class="x_panel ">
         <div class="x_content">
            ...
            <table class="table table-hover table-striped compact" id="tb_clientes">
               <thead>
                  <tr>
                     <th>Nome</th> 
                     <th>Telefone</th>
               ...
               <tbody>
                  @foreach($clientes as $key=> $cliente)
                     <tr>
                        <td>{{ $cliente->nome }}</td> 
                        <td>{{ formataTelefone($cliente->telefone1) }}</td>
                        <td>{{ $cliente->email }}</td>
...
@push("scripts")
   ...
   <script>
		$(document).ready(function(){
         ...
         //botão de exclusão
			$("table#tb_clientes").on("click", "#btn_exclui_cliente",function(){
				event.preventDefault();
				

				let id_cliente = $(this).data('cliente');
				let btn = $(this);
						...
						$.post("{{ url('cliente/') }}/"+id_cliente, {
							...
							id: 			id_cliente,
								...
								if(data =="ok"){
									...
									$("table#tb_clientes").DataTable().row( btn.parents('tr') ).remove().draw();
